 ...
 * LastEditTime : 2022-06-17 12:08:31
 * Description  : 
 */
...
namespace userlib {
    void enableCursor(uint8 strtLine, uint8 endLine) {
        uint8 tmp;
        syscall_portWrite(0x03D4, 0x0A);
        tmp = syscall_portRead(0x03D5);
        tmp = (tmp & 0xC0) | strtLine;
        syscall_portWrite(0x03D5, tmp);
        syscall_portWrite(0x03D4, 0x0B);
        tmp = syscall_portRead(0x03D5);
    ...
    void disableCursor() {
        syscall_portWrite(0x03D4, 0x0A);
        syscall_portWrite(0x03D5, 0x20);
    }

    uint16 getCursor() {
        uint16 position = 0;
        syscall_portWrite(0x03D4, 0x0E);
        position |= (uint16)(syscall_portRead(0x03D5)) << 8;
        syscall_portWrite(0x03D4, 0x0F);
        position |= syscall_portRead(0x03D5);
        return position;
    }
    ...
    void setCursor(uint8 row, uint8 col) {
        ...
        syscall_portWrite(0x03D4, 0x0E);
        syscall_portWrite(0x03D5, (uint8)((pos >> 8) & 0xFF));
        syscall_portWrite(0x03D4, 0x0F);
        syscall_portWrite(0x03D5, (uint8)(pos & 0xFF));
    }
    ...
    void setCursor(uint16 position) {
        ...
        syscall_portWrite(0x03D4, 0x0E);
        syscall_portWrite(0x03D5, (uint8)((position >> 8) & 0xFF));
        syscall_portWrite(0x03D4, 0x0F);
        syscall_portWrite(0x03D5, (uint8)(position & 0xFF));
    }
